...
void Sigma_update1(int dir, su3_matrix* sigma_off, su3_matrix* stp,
                   su3_matrix** l1, Real alpha1, Real alpha2, int sigfresh);
    ...
    if( dir3 != dir2) {
      for (dir = XUP; dir <= TUP; dir++) {
        if (dir3 != dir && dir != dir2) {
          ...
          compute_sigma23(SigmaH[dir], hyplink1[dir4][dir3],
                          hyplink1[dir4][dir], Lambda1[dir],
                          Lambda1[dir3], dir, dir3, EVENANDODD);
        }
      } // Loop over dir
...
void Sigma_update1(const int dir, su3_matrix *sigma_off, su3_matrix *stp,
  ...
  FORALLSITES(i, s) {
    ...
    ctmp = trace_su3(&SigmaOmega);
    ctmp1 = complextrace_su3(&Q, &SigmaOmega);
    ctmp2 = complextrace_su3(&Q2, &SigmaOmega);
    for (j = 0; j < 3; j++) {
    ...
    c_scalar_mult_su3mat(&Q, &traces[1], &tmat);
    c_scalar_mult_add_su3mat(&tmat, &Q2, &traces[2], &Gamma);
    c_scalar_add_diag_su3(&Gamma, &traces[0]);

    // The terms proportional to f_i
    scalar_mult_add_su3_matrix(&Gamma, &SigmaOmega, f[1], &Gamma);
    mult_su3_nn(&SigmaOmega, &Q, &tmat);
    scalar_mult_add_su3_matrix(&Gamma, &tmat, f[2], &Gamma);
...
// dir1 is the direction of the link and Sigma
void compute_sigma23(su3_matrix *sig, su3_matrix *lnk1, su3_matrix *lnk2,
                     su3_matrix *lambda1, su3_matrix *lambda2,
                     int dir1, int dir2, int parity) {
  ...
  FORSOMEPARITY(i, st, parity) {
    // Term 1
    mult_su3_nn(lambda2 + i, (su3_matrix *)gen_pt[1][i], &tmat1);
    mult_su3_na((su3_matrix *)gen_pt[0][i], &tmat1, sig + i);

    // Term 2
    mult_su3_nn((su3_matrix *)gen_pt[8][i],
                (su3_matrix *)gen_pt[4][i], &tmat1);
    mult_su3_an(&tmat1, (su3_matrix *)gen_pt[2][i], &tmat2);
    add_su3_matrix(sig + i, &tmat2, sig + i);

    // Term 3
    mult_su3_nn((su3_matrix *)gen_pt[3][i],
                (su3_matrix *)gen_pt[9][i], &tmat1);
    mult_su3_an(&tmat1, (su3_matrix *)gen_pt[2][i], &tmat2);
    add_su3_matrix(sig + i, &tmat2, sig + i);
    ...
    mult_su3_nn((su3_matrix *)gen_pt[3][i],
                (su3_matrix *)gen_pt[4][i], &tmat1);
    mult_su3_an(&tmat1, (su3_matrix *)gen_pt[7][i], &tmat2);
    add_su3_matrix(sig + i, &tmat2, sig + i);

    // Term 5
    mult_su3_na((su3_matrix *)gen_pt[5][i],
                (su3_matrix *)gen_pt[1][i], &tmat1);
    mult_su3_na(&tmat1, lnk1+i, &tmat2);
    add_su3_matrix(sig + i, &tmat2, sig + i);

    // Term 6
    mult_su3_na((su3_matrix *)gen_pt[0][i],
                (su3_matrix *)gen_pt[6][i], &tmat1);
    mult_su3_na(&tmat1, lnk1 + i, &tmat2);
    add_su3_matrix(sig + i, &tmat2, sig + i);
  }

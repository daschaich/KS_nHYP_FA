...
int meas_link(field_offset chi_off, field_offset psi_off, Real mass) {
  ...
  double first, second, qk_conn;      // Susceptibility terms
  complex cc, tc, W_ave, WSq_ave;
  ...
  for (jpbp_reps = 0; jpbp_reps < npbp; jpbp_reps++) {
    ...
    // Generate (one-mass) g_rand; chi_off = M g_rand; psi_off = M^{-1} g_rand
...
//      mat_invert_uml(F_OFFSET(M_inv), psi_off, chi_off, mass);
    ...
    // For susceptibilities (now that we have accumulated)
    CADD(pbp_e[0], pbp_o[0], cc);
    CDIVREAL(cc, (double)volume, cc);
    CSUM(W_ave, cc);      // W_ave += cc
    CMUL(cc, cc, tc);     // tc = cc * cc
    CSUM(WSq_ave, tc);    // WSq_ave += tc
  }
  ...
  // Two-flavor susceptibilities: print first two terms in Urs Eq. 22
  // Analysis needs to average each term over configurations,
  // then add them together with -V * <pbp>^2 = -4 * V * Si_{one-flavor}^2
  CDIVREAL(W_ave, (double)npbp, W_ave);
  CDIVREAL(WSq_ave, (double)npbp, WSq_ave);

  // Already divide by volume in W_ave
  CMUL(W_ave, W_ave, cc);
  first = volume * cc.real;

  // Connected susceptibility from Urs Eq. 17
  qk_conn = -volume * npbp / ((double)(npbp - 1)) * (WSq_ave.real - cc.real);
  second = 2 * qk_conn;     // Triplet from Urs Eq. 23

  node0_printf("susc: %.8g %.8g\n", first, second);

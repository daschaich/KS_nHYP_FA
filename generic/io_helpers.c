...
// -----------------------------------------------------------------
// Find out whether to gauge fix to Landau gauge
int ask_gauge_fix(FILE *fp, int prompt, int *flag) {
  int status = 0;
  char savebuf[256];

  if (prompt != 0)
    printf("enter 'no_gauge_fix', or 'landau_gauge_fix'\n");
  ...
  printf("%s\n", savebuf);
  if (strcmp("landau_gauge_fix", savebuf) == 0)
    *flag = LANDAU_GAUGE_FIX;
  else if (strcmp("no_gauge_fix", savebuf) == 0)
    *flag = NO_GAUGE_FIX;
  else {
    printf("Error in input: invalid gauge fixing_command\n");
    printf("Only no_gauge_fix and landau_gauge_fix supported\n");
    return 1;
  }
  return 0;
}

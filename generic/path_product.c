...
void path_product( const int *dir, const int length, matrix *tempmat1) {
	...
	*/
    ...
    tempmat3t = (matrix *)special_alloc( sites_on_node*sizeof(matrix) );
    tempmat2t = (matrix *)special_alloc( sites_on_node*sizeof(matrix) );
	...
	if( j%2==1 ){
	    if( GOES_FORWARDS(dir[j]) ) {
		  ...
		  mult_su3_nn( (matrix *)(gen_pt[0][i]), &(s->link[dir[j]]),
		    &(tempmat2t[i]) );
	        } END_LOOP

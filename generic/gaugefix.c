...
matrix *diffmatp;                       /* malloced diffmat pointer */
vector *sumvecp;                        /* malloced sumvec pointer */
...
/* in sumvec  */
  ...
  FORSOMEPARITY(i,s,parity)
      ...
      FORALLUPDIRBUT(gauge_dir,dir)
	{
	  ...
	  if(diffmat_offset >= 0)
	    add_matrix((matrix *)F_PT(s,diffmat_offset), m2, 
...
void gaugefix(int gauge_dir,Real relax_boost,int max_gauge_iter,
  ...
  int gauge_iter;
  double current_av, old_av, del_av = 0.0;

  // Require at least 8 gen_pt values for gauge fixing
  if (N_POINTERS < 8) {
    printf("gaugefix: N_POINTERS %d must be at least 8\n", N_POINTERS);
    fflush(stdout);
    terminate(1);
  }
  ...
  // Stop if the change in the avg gauge fixing action

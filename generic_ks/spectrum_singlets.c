...
void mult_eta_l(vector *src, vector *dest, vector *temp, matrix *smearlink) {
  ...
  for(c=0;c<6;c++) {
    ...
    FORALLSITES(i, s)
      scalar_mult_sum_vector(&(temp[i]), p[c].sign, &(dest[i]));
  }
}
...
void mult_flavor_pseudovector_smear(int mu, vector * src, vector * dest, matrix *smearlink) {
  ...
  register site *s;
  int p;
  vector *ltemp = malloc(sites_on_node * sizeof(vector));
  ...
  FORALLSITES(i, s)
   ...
   for(p=0;p<24;p++)
    ...
    FORALLSITES(i, s)
      scalar_mult_sum_vector(&(ltemp[i]), eps[p].sign/6.0, &(dest[i]));
  }
   free(ltemp);
}
...
void mult_flavor_pseudoscalar_smear(vector * src, vector * dest,
  ...
  * definition of the operator (number of permutations)   */
       FORALLSITES(i, s)
   scalar_mult_sum_vector(&(ltemp[i]), eps[p].sign/24.0, &(dest[i]));
     }
  free(ltemp);
}
...
void mult_spin_pseudovector_smear(int mu, vector * src, vector * dest,
  ...
  register site *s;
  int p;
  vector *ltemp = malloc(sites_on_node * sizeof(*ltemp));

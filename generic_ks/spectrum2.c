...
* modified 8/96 DT changed source and src_inc
...
*     src_start >= 0 with
*      src_start + n_src*src_inc < nt invokes corner wall src
*     src_start >= nt with
*      src_start + n_src*src_inc < 2*nt invokes point src
...
int spectrum2(Real vmass, field_offset temp1, field_offset temp2) {
  ...
  vmass_x2 = 2.0 * vmass;
  cgn = 0;
  pi_ps_prop = (double *)malloc(nt*sizeof(double));
  pi_sc_prop = (double *)malloc(nt*sizeof(double));
  rho_vt_prop = (double *)malloc(nt*sizeof(double));
  rho_pv_prop = (double *)malloc(nt*sizeof(double));
    ...
    for (icol=0; icol<3; icol++) {
      ...
      if (t_source < nt) {  /* wall source */
        ...
        if (node_number(0, 0, 0, t_source % nt) == mynode()) {
          ...
          ((su3_vector *)(F_PT(&lattice[i], temp1)))->c[icol].real = (-nx*ny*nz/8);
        }
        source_type |= 2;
      }

      /* do a CG (source in temp1, result in temp2) */
      if (t_source % 2 == 0) {
        cgn += ks_congrad(temp1, temp2, vmass, EVEN);
        dslash(temp2, F_OFFSET(ttt1[0]), ODD);    // Multiply by -M^dag
        scalar_mult_latvec(temp2, -vmass_x2, F_OFFSET(ttt1[0]), EVEN);
      }
      else {
        cgn += ks_congrad(temp1, temp2, vmass, ODD);
        dslash(temp2, F_OFFSET(ttt1[0]), EVEN);   // Multiply by -M^dag
        scalar_mult_latvec(temp2, -vmass_x2, F_OFFSET(ttt1[0]), ODD);
      }

      /* fill the hadron matrix */
      copy_latvec(F_OFFSET(ttt1[0]), F_OFFSET(propmat[icol]), EVENANDODD);
    } /* end loop on icol */
    ...
    for (t = 0; t < nt; t++) {
      ...
      for (x=0;x<nx;x+=2)for (y=0;y<ny;y+=2)for (z=0;z<nz;z+=2) {
        ...
        if ((((t + t_source) / nt - t_source / nt) % 2) == 0)
          barprop[t] += cc.real;
        else  /* change sign because antiperiodic b.c.
  ...
  if (this_node == 0) {
    ...
    for (t=0;t<nt;t++) printf("%d %e 0.0 %e 0.0 %e 0.0 %e 0.0\n",t,
      pi_ps_prop[t]/n_src, pi_sc_prop[t]/n_src,
      rho_vt_prop[t]/n_src, rho_pv_prop[t]/n_src);
    printf("ENDPROP\n");
